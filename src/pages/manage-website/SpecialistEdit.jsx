...
export default function SpecialistEdit({
  ...
  const handleImageUploadSection3 = async (e, type, ind) => {
    if (type == "img") {
      ...
      try {
        ...
        const imageData = await imageResponse.json();
        let inputData = section3?.awards.map(item => ({ ...item }));
        inputData[ind] = imageData.imageUrl;
        setSection3({ ...section3, awards: inputData });
        // return imageArr;
      } catch (error) {
        // toast.error("Error uploading image.");
        console.error("Error:", error);
    ...
    } else if (type == "experitise") {
      let inputData = section3?.experitise.map(item => ({ ...item }));
      inputData[ind] = e?.target?.value;
      setSection3({ ...section3, experitise: inputData });
    } else {
      let inputData = section3;
  ...
  return (
    <>
        ...
        <div className="row d-flex">
          <div className="about-us-container">
            ...
            <div>
              ...
              />
              <input type="file" onChange={e => handleImageUploadSection1(e)} />
            </div>
            <div>
              <ReactQuill
      ...
      <div
        className="about-section-2 d-flex row"
        style={{ flexDirection: "column" }}
      >
        {section2?.map((e, ind) => {
          return (
            <div key={`section2-item-${ind}`} className="section-item">
              <img alt="hair" src="/uploads/admprsp-icon1.png" />
              <div className="sec-item d-flex">
                <input
                  type="text"
                  defaultValue={e?.title}
                  onChange={e => handleImageUploadSection2(e, "title", ind)}
                  className="container-2-heading"
                />
                <p>
                  <textarea
                    defaultValue={e?.desc}
                    onChange={e => handleImageUploadSection2(e, "desc", ind)}
                  />
                </p>
              </div>
            </div>
      ...
      <div className="specialist-section-3 ">
        ...
        <div className="sec-3 sec-pad animate__backInRight">
          <input
            type="text"
            defaultValue={section3?.name}
            onChange={e => handleImageUploadSection3(e, "name", 0)}
          />
          <p>
            <input
              type="text"
              defaultValue={section3?.specialist}
              onChange={e => handleImageUploadSection3(e, "specialist", 0)}
            />
            <br />
            Experience:{" "}
            <input
              type="text"
              defaultValue={section3?.experience}
              onChange={e => handleImageUploadSection3(e, "experience", 0)}
            />
          </p>
        </div>
        <div className="sec-3 sec-pad1">
          {section3?.experitise?.map((e, indexx) => {
            return (
              <div key={indexx}>
                <input
                  type="text"
                  defaultValue={e}
                  onChange={e =>
                    handleImageUploadSection3(e, "experitise", indexx)
                  }
                />
              </div>
            );
      ...
      <div className="specialist-section-4 ">
          ...
          <div class="col-lg-8 col-md-8">
            <div class="row mb-20">
              ...
              <div class="col-lg-9">
                <input
                  type="text"
                  defaultValue={section3?.qualification}
                  onChange={e =>
                    handleImageUploadSection3(e, "qualification", 0)
                  }
                />
              </div>
            </div>
            <div class="row">
              ...
              <div class="col-lg-9">
                <p>
                  <input
                    type="text"
                    defaultValue={section3?.association}
                    onChange={e =>
                      handleImageUploadSection3(e, "association", 0)
                    }
                  />
                </p>
              </div>
      ...
      <div
        className="about-container-4 container"
        style={{
          margin: "30px 0 0 0",
        }}
      >
        <div className="container-4-item">
          <input
            ...
            onChange={e =>
              setSection5({ ...section5, title: e?.target?.value })
            }
          />

          <ReactQuill
            defaultValue={section5?.desc}
            onChange={e => setSection5({ ...section5, desc: e })}
          />
          ...
          {section5?.data?.map((item, index) => (
            <div key={index} className="container-4-item">
              ...
              <textarea
                ...
                onChange={e => handleImageUploadSection5(e, "desc", index)}
              />
            </div>
          ))}
        </div>
